...
class VGGModel(models.vgg.VGG):
    ...
    """
    def __init__(self, arch='vgg16', pretrained=True, requires_grad=True, remove_fc=True):
        ...
        super().__init__(self.make_layers(self.convlayer_configs[arch]))

        if pretrained:
            self.load_state_dict(getattr(models), arch)(pretrained=True).state_dict())
            print('Pretrained')
